...
class ProductListView(View):
    def get(self, request):
        ...
        gender_product = product_list.filter(
            product__menu_category_sub_category__menu_category__menu__name = menu_name
        ).distinct()
        DEFAULT_LIKES = 600

        # sizes
        product_color_size_objects = ProductColor.objects.prefetch_related(
            'productcolorsize_set').get(product_number = product_number)
        ...
        products = [
            {   
                ...
                'productImg'    : [
                    img.image_url for img in single_product.productimage_set.all()
                ]
...
class ProductDetailView(View):
    def get(self, request, p_num):
        ...
        # product_name
        related_product = ProductColor.objects.select_related(
            'product').get(product_number = p_num)
        product_name = related_product.product.name
        
        # sizes
        pcs = ProductColor.objects.prefetch_related(
            'productcolorsize_set').get(product_number = p_num)
        ...
        # images
        detail_product = ProductColor.objects.prefetch_related(
            'detailimage_set', 'product').get(product_number= p_num)
        ...
        review_info = [
            {
                ...
                'size': review.order.cart_set.first().size
            }
